...
import net.mcreator.thestarwarssagamod.procedures.TatoniePlayerEntersDimensionProcedure;
import net.mcreator.thestarwarssagamod.particle.CartredgeParticle;
import net.mcreator.thestarwarssagamod.item.TatonieItem;
...
public class TatonieDimension extends TheStarWarsSagaModModElements.ModElement {
	@ObjectHolder("the_star_wars_saga_mod:tatonie")
	public static final ModDimension dimension = null;
	@ObjectHolder("the_star_wars_saga_mod:tatonie_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public TatonieDimension(TheStarWarsSagaModModElements instance) {
		super(instance, 84);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("tatonie"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("the_star_wars_saga_mod:tatonie")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("the_star_wars_saga_mod:tatonie"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("the_star_wars_saga_mod:tatonie"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new TatonieItem().setRegistryName("tatonie"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("tatonie_portal");
		}
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(CartredgeParticle.particle, px, py, pz, vx, vy, vz);
			}
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = TatonieDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == Blocks.CHISELED_SANDSTONE.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == Blocks.CHISELED_SANDSTONE.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == Blocks.CHISELED_SANDSTONE.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("tatonie_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "tatonie_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = TatonieDimension.CustomPortalBlock.createPatternHelper(this.world, blockpos);
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? Blocks.CHISELED_SANDSTONE.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.CHISELED_SANDSTONE.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		if (event.getTo() == type) {
			{
				...
				TatoniePlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
